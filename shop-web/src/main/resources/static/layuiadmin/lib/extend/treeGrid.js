...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        ,layer = layui.layer
        ,form = layui.form
        ,hint = layui.hint()//打印日志使用
        ,device = layui.device()//获得操作系统和操作系统环境的信息
        //外部接口
        ,table = {
            config: {
        ...
        if(options.url){ //Ajax请求
            ...
            $.ajax({
                ...
                ,success: function(res){
                    var data = that.filterArray(res.data,options.treeId,options.treeParentId);
                    res.data=data[0];
            ...
            console.log(options.data,options.treeId,options.treeParentId);
            var data = that.filterArray(options.data,options.treeId,options.treeParentId);
            res.data=data[0];
